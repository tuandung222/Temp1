...
sidebar_position: 1
description: Giới thiệu cụm Static Analysis II về concurrency verification, vì sao multi-threaded khó hơn nhiều so với sequential, và các kỹ thuật chính giảm state explosion.
---
...
Đây chính là **state explosion problem trong concurrency**, mạnh hơn cả state explosion mà chúng ta đã thấy trong [bài 3.3](../02-static-analysis-i/03-state-space-exploration). Cụm Lecture 4 tập trung vào các kỹ thuật để **vượt qua** state explosion này.
